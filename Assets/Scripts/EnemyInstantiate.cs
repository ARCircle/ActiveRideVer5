	...
	void Update () {
		...
		if (timer < 0) {
			...
			if(instantiateValue > 0){
				//敵をランダムな位置に生成
				Instantiate(enemy, new Vector3(Random.Range (-100.0f, 100.0f), Random.Range (7, 15.0f), Random.Range (-100.0f, 100.0f) ), Quaternion.identity);
				instantiateValue --;
				Debug.Log (instantiateValue);
			}
